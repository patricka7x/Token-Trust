...
STRIPE_SUCCESS_URL = os.getenv("STRIPE_SUCCESS_URL", "https://yourdomain.com/success")  # Configurable
STRIPE_CANCEL_URL = os.getenv("STRIPE_CANCEL_URL", "https://yourdomain.com/cancel")    # Configurable
DEV_USER_ID = int(os.getenv("DEV_USER_ID", 0))  # Your Telegram user ID for /reset command
FALLBACK_PREMIUM_LINK = "https://buy.stripe.com/your-static-link"  # Add a static fallback link here if desired
...
c.execute('''CREATE TABLE IF NOT EXISTS users
             (user_id INTEGER PRIMARY KEY, is_premium BOOLEAN DEFAULT FALSE, subscription_id TEXT, expiry_date DATETIME, total_scans INTEGER DEFAULT 0)''')
c.execute('''CREATE TABLE IF NOT EXISTS alerts
             (user_id INTEGER, addr TEXT, set_value TEXT, percent REAL, direction TEXT, name TEXT, alert_type TEXT, PRIMARY KEY (user_id, addr))''')
conn.commit()

# FastAPI app for Stripe webhook (moved earlier to ensure global scope)
api_app = FastAPI()
...
async def webhook(request: Request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        # Assuming monthly subscription; adjust timedelta as needed
        ...
        conn.commit()
        logger.info(f"Premium activated for user {user_id} with subscription {sub_id}")
...
def can_scan(user_id):
    ...
    if not row:
        ...
        return True
    total_scans = row[0]
    if is_premium(user_id) or total_scans < 3:
...
def create_checkout_session(user_id):
    try:
        ...
        print(f"Stripe Error: {e}")  # Print to console for easy debugging
        return None
    ...
    if not can_scan(user_id):
        ...
        if checkout_url:
            await update.message.reply_text(
                f"You've used your 3 free scans. Upgrade to premium for unlimited scans and price/liquidity alerts! **[Subscribe here]({checkout_url})**",
                parse_mode="Markdown"
            )
        else:
            # Fallback with static link if dynamic fails
            await update.message.reply_text(
                f"You've used your 3 free scans. Upgrade to premium for unlimited scans and price/liquidity alerts! **[Subscribe here]({FALLBACK_PREMIUM_LINK})** (Error generating dynamic link - check logs for details.)",
                parse_mode="Markdown"
            )
        return
...
def main():
    try:
        ...
        thread.start()
        print("FastAPI webhook started on port 8000.")  # Dev feedback
    except Exception as e:
        logger.error(f"Failed to start FastAPI: {e}")
        print(f"Error starting FastAPI: {e} - Continuing with Telegram bot only.")

    app = ApplicationBuilder().token(TELEGRAM_TOKEN).build()
    app.add_handler(CommandHandler("start", start))
    app.add_handler(CommandHandler("reset", reset_scans))  # New command
